...
Task 7 完成“矛盾记忆重演”的运行时边界。饭桌的三段错误重演只在完整播放后才把对应证据写入 `seenMealContradictions`，重复调查使用 720ms 的短回放；正确座次只有在 5200ms 的完整家庭重演结束后才提交 `tableSolved` 与侧门开启。这样玩家不是读一条提示就拿到进度，而是先看见矛盾如何发生，再让世界状态改变。

这类重演最容易留下“结束了但场景没复原”的隐蔽错误：补间若直接作用于椅子、门、污渍和碎片等场景原物，shutdown、换图或中断时必须先停掉 timer/tween，再恢复所有实际被写过的属性、销毁临时演员与特效，最后依持久状态重新刷新门。不能只检查标记物是否生成；动态验证必须把 tween 推到终态后再中断，确认真实对象坐标、透明度和锁门外观都还原。注册表查询也必须用 `Object.hasOwn`，避免 `toString` 一类原型属性被误作记忆 ID。

Task 7 的独立规格与质量复审均未发现阻塞问题；当时新鲜执行的专项、全量 `35/35` verifier、`31/31` 源码语法检查和 `git diff --check` 均通过。自动化仍不能替代真实 Phaser 渲染下的节奏、补间感受与盲测，因此它们只证明开发候选版的逻辑边界，不证明体验已经成立。

Task 8 把饭铃从随机惩罚改成了可学习的因果链：正确饭局后的第一铃只教学，不会因移动处罚；第二铃只在开放侧门附近触发，按照饭铃、敲门、脚步、门影、检查的固定时序推进。对话、记忆重演、搬碗和切房时不累计时间或位移；桌下静止可躲过检查，继续移动才暴露。注意度必须真正经过 `quiet → suspicious → checking → chasing`：从可疑升到检查时，父影在最后经过的门短暂停留，不能把这一阶段跳过去直接追人。

切片追逐与旧路线隔离。`startSlice()` 只接收当前切片地图和明确门 ID，用 2 秒门响、3 秒脚步、4 秒安全格入门模拟跨图追赶；成功入门后才开始完整 10 秒追逐，无安全出生格也在有界 setup 截止后清理。失败回到本次铃响前的检查点，保留座次、记忆、家规教学和纸飞机选择。切片追逐期间必须禁止重开饭铃，否则会再次锁住出口或错误回落注意度。

本次审查还确认一个对长玩稳定性很重要的规则：销毁显示对象不等于销毁其无限补间。门光与脚步的 `repeat:-1` tween 必须由切片追逐自己保存引用，并在超时、抓到、场景销毁和重试收口里 stop/remove；动态回归覆盖连续多轮追逐，且不得误杀 legacy tween。Task 8 最后由主线程新鲜执行 `36/36` verifier、`33/33` 源码语法检查、单文件重建/验证和提交范围 diff 检查。真实浏览器下的压迫节奏、性能和玩家理解仍留给后续 smoke 与盲测，不能被这些结果替代。

## 2026-07-15

完成一次独立“游戏高手”审计，并与源码、18 张地图拓扑、七个谜题、剧情状态、恐怖系统、结局路由和既有试玩记录交叉核对。审计 agent 只能确认本地构建和标题入口正常，内置浏览器运行时遇到 `Cannot redefine property: process`，没有完成无先验连续通关；因此完整流程判断明确按源码、拓扑和既有记录推断，不能冒充真实盲测。核心结论是：上一轮把工程复杂度误当成推理深度，把剧情信息量误当成玩家经历。七题虽然换了题材，底层仍然是 `slots + tokens + answer` 的固定配对；证据板又同时铺开全部线索、槽位和选项，玩家实际执行的是“找发光点、读明示文本、配同义词、等主角公布答案”。自动化能证明状态可达和判定稳定，不能证明玩家真的观察、推理、害怕或记住剧情。

老板批准采用激进内容重做，但不重写 Phaser 2D 技术底座。正式规格沉淀为 `docs/superpowers/specs/2026-07-15-huisha-vertical-slice-redesign-design.md`：第一阶段只做正厅、厨房、孩子卧室的 15–20 分钟切片，用实体饭桌座次、矛盾记忆重演、可学习的饭铃家规和有即时得失的纸飞机选择，验证“观察环境—操作物件—世界回应—承担代价”的新核心闭环。统一证据板、供品清单、随机惊吓、隐形理智和完成度结局不再作为新路线基础。

本轮最重要的可复用边界是：体验目标必须有同尺度证据。脚本、语法、路由和浏览器控制台只能验工程；首次理解时间、是否暴力枚举、规则能否从环境学会、反转是否过早、玩家能否回忆具体意象，必须由无先验连续盲测验证。切片明确以五名陌生玩家为放行门槛，任何核心指标失败都不得扩写全流程。旧版继续由 `codex/2d-rework` 和 Git 历史归档，新切片在后续独立功能分支实施，避免为兼容旧清单循环污染新设计。

实施准备已放入项目内隔离 worktree `.worktrees/vertical-slice-redesign`，对应分支为 `codex/vertical-slice-redesign`；`.worktrees/` 已加入主工作区忽略规则，避免嵌套工作树进入版本控制。新工作树先重建单文件入口，再运行当时全部 28 项 `tools/verify_*.mjs`、24 个 `src/**/*.js` 语法检查与 `git diff --check`，基线均通过。逐任务施工蓝图位于 `docs/superpowers/plans/2026-07-15-huisha-vertical-slice-redesign.md`，按纯状态与路由、三空间与实体饭桌、矛盾重演与饭铃规则、纸飞机代价、运行时隔离、双路线验收和五人盲测七层拆开，每层先写失败契约、再实现、再独立提交。计划阶段的边界是不提前宣称体验成立：自动化和开发者双视口烟测只产生开发候选版，能否理解、紧张、记住和愿意讨论选择仍以五名无先验玩家数据为准。此前推送归档分支时遇到 `schannel: failed to receive handshake`，本地提交未丢失；网络失败时应保留提交并重试，不能为了推送改写历史。

Checkpoint A 的跨模块审计补齐了几个可复用的运行时边界。存档恢复必须是语义归一化边界：不得盲目展开持久化对象，门 ID、阶段、布尔量、枚举、碗摆放与派生解谜结果要交叉验证，未声明字段一律丢弃。门身份必须从冻结地图定义派生共享注册表，不能在状态、地图和追逐中各写一套字符串。饭桌的实体碰撞与桌下藏身区是两种不同几何：前者进入导航阻挡，后者必须保持可走。`sliceMode` 不是入口临时参数，而是需跨每次重启、切房和失败恢复传递的路线命名空间；追逐适配器也不得硬编码旧地图尺寸、旧出生点或旧叙事台词。单文件构建会剥离 import，因此验证器要约束“依赖先于使用者”的真实生成顺序，不能只做语法 smoke。

Task 5 已完成三空间可运行渲染层。`SliceMapManager` 对冻结的地图定义做运行时深拷贝，向旧 `GameScene` 暴露兼容的 walls / doors / furniture / interactables / floor / navigation surface；饭桌实体矩形进入导航阻挡，桌下区始终是独立运行时元数据。玩家在卧室做出选择后，厨房重进会恢复真实世界后果：带走纸飞机保留捷径但取消桌下保护、暖光与饭汽；留下纸飞机保留保护却反锁侧门。revision effect 与 ambient effect 分离管理，重复应用幂等，scene shutdown 显式销毁且不依赖 Phaser 隐式回收。

这一任务还证明：锁门是谜题反馈的一部分，不能复用不同路线的通用文案。新切片锁门若继续弹“需要银色钥匙”，会把座次门禁误导成不存在的找钥匙任务。现在 slice 锁门只给 115Hz / 0.08s 短门响与 3px / 60ms 门体回弹，900ms 节流，不弹字、不抖屏、不检查钥匙；legacy 银钥匙路径保持不变。动态验证覆盖三图、五门、两次连续切房、三种厨房修订、道具纹理、shutdown 和 legacy 回归；当时全量为 33/33 verifier 与 28/28 源码语法检查。已知但不阻塞的边界是：`SliceMapManager.createMap()` 尚不支持同一 scene 原地重建；切片缺图的受控回退依计划在 Task 10 完成。

Task 6 已用独立 `SliceInteractionManager` 和 `KitchenTableController` 替换新路线的旧证据板交互。切片候选先以 physics bounds 距离、对象 radius 和 28px 焦点带收窄，再用归一化朝向限制到正面半平面；背后唯一物件必须退出候选，中心重叠、零朝向和初始缺朝向则保留近场兜底。端碗时所有其他碗暂时让出焦点，由座位 hotspot 接管，否则占座碗会让“替换这个座位”在物理上无法选中。碗的 held / placed 切换会同步 `StaticBody.enable` 与 `updateFromGameObject()`，被替换碗回到自己原点，不复制、不丢失，供碗始终不可拿。

键盘“刚按下”是必须消费的边沿事件，不能用 `JustDown(E) || JustDown(Space)` 短路：两键同帧时第二个状态会残留到下一帧，可造成端起后自动放下。同理，用 Space 关闭观察对话后，500ms 防重触冷却必须先消费 pending key，再隐藏 prompt 并清空 target，否则对话会“刚关又弹回”。键盘 E / Space 与移动端按钮现在共用一个多态 `handleInteraction()` 入口，legacy 仍使用原 `InteractionManager`。当时全量为 34/34 verifier 和 30/30 源码语法检查；真实浏览器视觉手感仍需后续集成 smoke 与盲测，不由 mock 通过代替。

## 2026-07-12

完成解谜、沉浸与行为型恐怖的纵向重构。厨房死交互的根因不是半径不足，而是旧评分 `priority * 1000` 让远处高优先物件绝对压过眼前物件；现在先取物理边缘最近值，只在最近值加 28 像素的焦点带内比较剧情优先级和朝向。饭柜三个可达方向均稳定命中，提示显式为“检查：封死的饭柜”。密集房间以后应继续使用焦点带，不要靠移动物件或无限提高半径修抢占。

新增 `PuzzleState.js` 和 board 配置。槽位赋值不可变，同一 token 自动从旧槽移除；`evaluatePuzzle()` 区分缺前置、未填满、证据冲突和正确。运行进度统一保存在 `storyFlags.puzzleProgress`，案件结论进入 `caseConclusions`，奖励由 `applyPuzzleOutcome()` 幂等发放。厨房、密室、井绳、阁楼、学校、医院和供桌共七套证据板复用同一 UI；第 2/4 次错误分级提示，错误不消耗供品、不锁门。

新增 `HauntingDirector.js`。可复用边界是：纯函数判断“背视移动”和“数脚步”，运行层只管理作者节点、实体、计时与清理；对话、谜题、切图和追逐时暂停。厨房纸人背对才移动，三个监听节点静止 2.5 秒通过、移动 16 像素被察觉，失败只造成短时反馈和 10 点理智损失。scene shutdown 必须销毁文本、影子和 delayedCall，不能让恐怖事件跨图残留。

浏览器走查抓到证据板成功回调的输入锁顺序错误：若先 `onSuccess()` 打开对话再关闭谜题，会把新对话的 `dialogActive` 清掉。固定顺序必须是先关闭谜题自身，再打开后续对话。另将厨房水槽长氛围文缩成可观察事实，并在学校/医院案件结论成立后抑制旧返回独白，避免成功文本、返回独白、剧情回响连续复述同一结论。详细复盘见 `docs/playtests/2026-07-12-full-route.md`。
