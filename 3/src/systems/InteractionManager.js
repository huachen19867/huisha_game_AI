    ...
    checkInteraction() {
        ...
        if (!target && scene.interactables) {
            ...
            if (closestObj) {
                ...
                if (obj === scene.safe || obj.objId === 'safe' || (obj.texture && obj.texture.key === 'safe')) type = 'safe';
            ...
            if (type === 'cabinet') {
                ...
                if (obj.objId === 'kitchen_cabinet') {
                    if (!this.gameState.hasRice) {
                        window.showDialog('（内心独白）', '柜子里有一碗发馊的饭...这是给死人的供品。你获得了【倒头饭】。', () => {
...
`;
            ...
            if (type === 'family_rules') {
                if (!this.gameState.clues.includes('rules')) this.gameState.clues.push('rules');
                return;
            }
            ...
            if (type === 'safe') {
                ...
                window.showSafeUI(() => {
                    ...
                    window.showDialog('（内心独白）', '密码正确！保险箱打开了。', () => {
                        window.showDialog('（内心独白）', '里面只有一把冰冷的【地下室钥匙】。', () => {
            ...
            if (type === 'red_key') {
                ...
                window.showDialog('（内心独白）', '你捡起了那把【血红钥匙】...上面还沾着未干的血迹。', () => {
                    if (!this.gameState.isChasing) {
                        this.gameState.isChasing = true;
                        scene.spawnChaser();
                        window.showDialog('（内心独白）', '（周围的温度突然降低了...有什么东西来了！）');
                    }
                });
                return;
            }
            ...
            if (type === 'exit_door') {
                 if (this.gameState.clues.length >= 3) {
                     ...
                     window.showDialog('主角', '前面是出口...但我感觉还有些事情没弄清楚。是直接离开，还是回去打开那口棺材？', () => {
                        window.showDialog('（内心独白）', '（提示：集齐3个线索可以直接离开达成【破茧】结局；或者持有血红钥匙打开棺材达成【真相】结局）');
                     });
                 }
                 return;
            }
            ...
            if (type === 'altar') {
                if (this.gameState.hasRice && this.gameState.hasMatches && this.gameState.hasIncense && this.gameState.hasSpiritMoney) {
                    ...
                    window.showDialog('（内心独白）', '（你摆上了倒头饭，点燃了香和蜡烛，烧了纸钱。火苗诡异地跳动了一下。）', () => {
            ...
            if (type === 'photo') {
                ...
                if (!this.gameState.viewedPhotos.includes(photo.photoId)) {
                    ...
                    // Removed progressive darkening as per user request ("不要突然把环境变黑")
                    // scene.lights.setAmbientColor(0x444444 - (this.gameState.viewedPhotos.length * 0x111111));
            ...
            if (type === 'black_cloth') {
                 if (!this.gameState.doorSlammed) {
                     window.showDialog('（内心独白）', '（突然，身后传来“砰”的一声巨响！大门关上了。）', () => {
            ...
            */
            ...
            if (type === 'chest') {
                 if (!this.gameState.hasRice || !this.gameState.hasMatches || !this.gameState.hasIncense || !this.gameState.hasSpiritMoney || !this.gameState.hasRedKey) {
                     window.showDialog('（内心独白）', '你发现了【开发者宝箱】！获得了：倒头饭、火柴、香、纸钱、血红钥匙。', () => {
                         this.gameState.hasRice = true;
                         this.gameState.hasMatches = true;
                         this.gameState.hasIncense = true;
                         this.gameState.hasSpiritMoney = true;
                                        ...
                                        window.showDialog('尸体', '为什么...不救我...你看着我死的...你就站在那里...看着我割开了手腕...', () => {
                                            window.showDialog('主角', '不！不是这样的！我被关起来了！我出不去！', () => {
                                                window.showDialog('尸体', '撒谎...把钥匙给我...（猛地扑上来）', () => {
                                                    ...
                                                    this.gameState.hasRedKey = true;
                                                    window.updateInventory('血红钥匙');
                                                    window.showDialog('（内心独白）', '你拼命挣扎，在混乱中抓住了井边铁钩上的一把【血红钥匙】，并将尸体踹回了井里。', () => {
